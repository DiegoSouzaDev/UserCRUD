...
public class UserServiceTest {
	...
	public void testCreateOneUser() throws Exception {
		...
		when(persist.create(user)).thenReturn(index);
	...
	public void testCreateWithDuplicatedUsername() throws Exception {
		...
		when(persist.userNameAlreadyExist(user.getUserName())).thenReturn(true);
	...
	public void testFindUserByID() throws Exception {
		...
		when(persist.findByID(id)).thenReturn(getUser(id));
	...
	public void testSucessfulyUpdateUser() throws Exception {
		...
		when(persist.findByID(user.getId())).thenReturn(user);
		when(persist.update(user)).thenReturn(user);

		final User userUpdated = service.updateUser(user);

		Assert.assertEquals(user.getEmail(), userUpdated.getEmail());
		verify(persist, times(1)).findByID(user.getId());
		verify(persist, times(1)).update(user);
	}
	...
	public void testDeleteUser() {
		...
		verify(persist, times(1)).delete(id);
	}
